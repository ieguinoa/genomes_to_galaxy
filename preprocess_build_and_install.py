...
for line in genome_builds_file.readlines():
    ...
    print("------------------------")
    print("Creating transcriptome fasta file with command:")
    print("gffread -g %s -w $exome_out %s",out_genome)
    #gffread -g $genome_file -w $exome_out $gff_full

    # Get tx2gene and representative_tx_gff
    print("------------------------")
    print("Processing the annotation file: extracting longest transcript per gene and transcript to to gene table:")
    gff_processing(out_gff_path, tx2gene_out_path, longest_tx_out_path)
